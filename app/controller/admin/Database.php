...
class Database extends InitController
{
    ...
    public function database()
    {
        ...
        if (Request::param('getList') == 1) {
            ...
            if (Request::param('name')) {
                foreach ($list as $k => $v) {
                     if (strpos($v['name'], Request::param('name')) == true) {
                        $temp=$list[$k];
                        unset($list);
                        $list[]=$temp;
                    }
        ...
        return TableBuilder::getInstance()
            ...
            ->addTopButtons([
                'backup' => [
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => '$.operate.database(\''.url('backup/backup').'\' , \'备份\')',
                ],
                'optimize' => [
                    'title'       => '优化',
                    'icon'        => 'fa fa-medkit',
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => '$.operate.database(\''.url('backup/optimize').'\', \'优化\')',
                ],
                'repair' => [
                    'title'       => '修复',
                    'icon'        => 'fa fa-user-md',
                    ...
                    'href'        => '',
                    'target'      => '',
                    'onclick'     => '$.operate.database(\''.url('backup/repair').'\', \'修复\')',
                ],
            ])
            ->fetch();
    }
    ...
    public function restore()
    {
        ...
        // 提示信息
        $pageTips = '备份文件列表中共有 ' . count($list) . ' 个文件，共计 ' . format_bytes($total * 1024 * 1024);
       ...
       // 构建页面
        return TableBuilder::getInstance()
            ...
            ->addRightButton('info', [
                ...
                'href'  => url('backup/import', ['id' => '__time__'])
            ]) // 添加额外按钮
            ->addRightButton('info', [
                'title' => '下载',
                'icon'  => 'fa fa-download',
                ...
                'href'  => url('backup/downFile', ['id' => '__time__'])
            ]) // 添加额外按钮
    ...
    public function import(string $id)
    {
        ...
        $result = json(['error' => 0, 'msg' => '还原成功！']);
        return $result ? $this->success('', '','还原成功！') : $this->error('还原失败！');
    }
